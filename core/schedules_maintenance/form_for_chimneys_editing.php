...
<p align="center"><strong>Форма для редактирования расписания проверки дымовых и вентиляционных каналов</strong></p>

<?php

include_once ($_SERVER['DOCUMENT_ROOT']."/core/schedules_maintenance/class_schedules.php");
$schedules = new ClassSchedules();
$addible_addresses = $schedules->getAllAddressesWhichMustHaveScheduleButHavent("CHIMNEYS");
...
$all_data = $schedules->getScheduleDataForSetup("CHIMNEYS");
...
foreach ($all_data as $key => $row)
{
    ...
    echo "<tr $row_colour_str><form action=\"/core/schedules_maintenance/handler_chimneys.php\" method=\"POST\">";
    ...
    echo "<td><input size='4' name='PG' value='".$row["PG"]."'></td>";
    echo "<td><input size='4' name='GK' value='".$row["GK"]."'></td>";
    echo "<td><input size='4' name='boiler' value='".$row["boiler"]."'></td>";
    echo "<td><input size='15' name='material' value='".$row["material"]."'></td>";
